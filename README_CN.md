...
将已安装的程序文件更新到 GitHub 最新稳定版：

```sh
tmux-web update
```

该命令会自动识别当前可执行文件路径和 Linux 架构，将 `tmux-web -V` 返回的版本
与最新稳定版 tag 进行语义化比较，仅在最新版本更高时下载；下载内容会使用
Release 中的 SHA-256 文件校验，然后原子替换磁盘上的程序文件。它不会停止或重启
正在运行的服务；更新后需要手动重启服务才能使用新版本。执行命令的用户必须拥有
程序所在目录的写权限。

Release 产物是未压缩的二进制文件。手动下载时，请使用对应的 `.sha256` 文件完成
校验，并在运行前通过 `chmod +x` 添加执行权限。
